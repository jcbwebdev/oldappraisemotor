<?php
    ...
    /**
     * Deals with Letter items
     *
     * It will allow you to
     *  - retrieve an individual letter
     *  - retrieve an array of all letters
     *  - delete letter
     *
     * Relies on the Letter table in this structure:
     *  ID
     *  Title
     *  Content
     *  PanelShow
    ...
    class Letter
    {
        ...
        public function __construct($id = null)
        {
            ...
            if (!$this->_dbconn)
            {
                ...
                {
                    throw new Exception('Class Letter requires id to be specified as an integer');
                }
        ...
        public function getItemById($id)
        {
            try
            {
                $stmt = $this->_dbconn->prepare("SELECT * FROM Letters WHERE ID =  :id LIMIT 1");
                $stmt->execute(['id' => $id]);
                $story = $stmt->fetch();
            } catch (Exception $e)
            {
                error_log("Failed to retrieve Letter item details when searching by ID" . $e);
            }
        ...
        public function getItemByUrl($urltext)
        {
            try
            {
                $stmt = $this->_dbconn->prepare("SELECT ID FROM Letters WHERE URLText LIKE :urltext LIMIT 1");
                $stmt->execute(['urltext' => $urltext]);
                $story = $stmt->fetch();
            } catch (Exception $e)
            {
                error_log("Failed to retrieve Letter item details when searching by URLText" . $e);
            }
        ...
        public function saveItem()
        {
            ...
            try
            {
                $stmt = $this->_dbconn->prepare("UPDATE Letters SET Title = :title, Content = :content, PanelShow = :panelshow, PanelExpire = :panelexpire, DateDisplay = :datedisplay, URLText = :urltext, MetaDesc = :metadesc, MetaKey = :metakey, MetaTitle = :metatitle, AuthorID = :authorid, AuthorName = :authorname WHERE ID = :id LIMIT 1");
                $result = $stmt->execute([
                                             'title' => $this->_title,
            ...
            {
                error_log("Failed to save Letter record: " . $e);
            }
        }

        /**
         * Create new empty letter item
         *
        ...
        public function createNewItem()
        {
            ...
            {
                throw new Exception('You cannot create a new Letter item at this stage - the id property is already set as ' . $this->_id);
            }

            //Create DB item
            try
            {
                $result = $this->_dbconn->query("INSERT INTO Letters SET Title = ''");
            ...
            {
                error_log("Failed to create new Letter record: " . $e);
            }
        }

        /**
         * Returns all letter records and fields in Assoc array
         ...
         * @return mixed
         */
        public function getAllLetterItems($flagWhichItems = 'All', $CutOffInMonths = null)
        {
            switch ($flagWhichItems)
            {
                case 'Panel':
                    $sql = "SELECT * FROM Letters WHERE DateDisplay <= NOW() AND PanelShow = 'Y' AND PanelExpire >= NOW()";
                    break;
                case 'Current':
                    $sql = "SELECT * FROM Letters WHERE DateDisplay <= NOW()";
                    break;
                case 'All':
            ...
            {
                error_log("Failed to retrieve Letter items" . $e);
            }
        ...
        /**
         * Delete the complete letter item - including any images
         *
         * @return mixed
         */
        public function deleteItem()
        {
            ...
            {
                throw new Exception('Class Letter requires the letter item ID to be set if you are trying to delete the item');
            }

            //Now delete the item from the DB
            try
            {
                $stmt = $this->_dbconn->prepare("DELETE FROM Letters WHERE ID = :id LIMIT 1");
                $result = $stmt->execute([
                                             'id' => $this->_id
                                         ]);
            } catch (Exception $e)
            {
                error_log("Failed to delete Letter record: " . $e);
            }
        ...
        public function URLTextValid($ID = 0, $ContentURL)
        {
            ...
            {
                throw new Exception('Letter needs the new URL specifying as a string');
            }


            if (clean_int($ID) > 0)
            {
                $sql = "SELECT ID FROM Letters WHERE URLText = :urltext AND ID != :id";
                $vars = array('urltext' => $ContentURL, 'id' => $ID);
            }
            else
            {
                $sql = "SELECT ID FROM Letters WHERE URLText = :urltext AND (ID IS NULL OR ID <= 0)";
        ...
        function searchContent($needle = '')
        {
            ...
            //Return IDs
            $sql = "SELECT ID FROM Letters WHERE (Title LIKE :needle OR Content LIKE :needle) AND DateDisplay <= NOW() AND DATE_ADD(DateDisplay, INTERVAL 6 MONTH) > NOW()";
            $stmt = $this->_dbconn->prepare($sql);
            $stmt->execute([
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC))
            {
                ...
                if ($content['URLText'] != '')
                {
                    $link = "//" . SITEFQDN . "/letter/" . $content['URLText'];
                }
                else
                {
                    $link = "//" . SITEFQDN . "/content/letterview.php?id=" . $content['ID'];
                }
