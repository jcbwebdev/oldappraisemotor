<?php
    ...
    /**
     * Deals with Calendar items
     *
     * It will allow you to
     *  - retrieve an individual calendar entry
     *  - retrieve an array of all calendar entries
     *  - delete entry
     *
     * Relies on the Calendar table in this structure:
     *  ID
     *  Title
     *  Content
     *  PanelShow
     ...
     *
     * @author Peter Bourne
     * @version 1.1     21.09.2021  Better handling of dates and error on save
     *
     */
    class Calendar
    {
        ...
        public function __construct($id = null)
        {
            ...
            if (!$this->_dbconn)
            {
                ...
                {
                    throw new Exception('Class Calendar requires id to be specified as an integer');
                }

                //Retrieve current letter information
                if (isset($id))
                {
                    $this->_id = $id;
                    $this->getItemById($id);
        ...
        /**
         * Retrieves specified letter record ID from Letter table
        ...
        public function getItemById($id)
        {
            try
            {
                $stmt = $this->_dbconn->prepare("SELECT * FROM Calendar WHERE ID =  :id LIMIT 1");
                $stmt->execute(['id' => $id]);
                $story = $stmt->fetch();
            } catch (Exception $e)
            {
                error_log("Failed to retrieve Calendar item details when searching by ID" . $e);
            }
        ...
        public function getItemByUrl($urltext)
        {
            try
            {
                $stmt = $this->_dbconn->prepare("SELECT ID FROM Calendar WHERE URLText LIKE :urltext LIMIT 1");
                $stmt->execute(['urltext' => $urltext]);
                $story = $stmt->fetch();
            } catch (Exception $e)
            {
                error_log("Failed to retrieve Calendar item details when searching by URLText" . $e);
            }
        ...
        /**
         * Saves the current object to the Letter table in the database
         *
         * @throws Exception
         */
        public function saveItem()
        {
            //First need to determine if this is a new Letter item
            if ($this->_id <= 0)
            {
            ...
            try
            {
                $stmt = $this->_dbconn->prepare("UPDATE Calendar SET Title = :title, Content = :content, PanelShow = :panelshow, PanelExpire = :panelexpire, DateDisplay = :datedisplay, URLText = :urltext, MetaDesc = :metadesc, MetaKey = :metakey, MetaTitle = :metatitle, AuthorID = :authorid, AuthorName = :authorname WHERE ID = :id LIMIT 1");
                $result = $stmt->execute([
                                             'title' => $this->_title,
            ...
            {
                error_log("Failed to save Calendar record: " . $e);
            }
        }

        /**
         * Create new empty calendar item
         *
        ...
        public function createNewItem()
        {
            ...
            {
                throw new Exception('You cannot create a new Calendar item at this stage - the id property is already set as ' . $this->_id);
            }

            //Create DB item
            try
            {
                $result = $this->_dbconn->query("INSERT INTO Calendar SET Title = ''");
            ...
            {
                error_log("Failed to create new Calendar record: " . $e);
            }
        }

        /**
         * Returns all calendar records and fields in Assoc array
         ...
         * @return mixed
         */
        public function getAllItems($flagWhichItems = 'All', $CutOffInMonths = null)
        {
            switch ($flagWhichItems)
            {
                case 'Panel':
                    $sql = "SELECT * FROM Calendar WHERE DateDisplay <= NOW() AND PanelShow = 'Y' AND PanelExpire >= NOW()";
                    break;
                case 'Current':
                    $sql = "SELECT * FROM Calendar WHERE DateDisplay <= NOW()";
                    break;
                case 'All':
            ...
            //Limit the results returned? Only relevant for Current and Past items (commonly used on letter index display page)
            ...
            {
                error_log("Failed to retrieve Calendar items" . $e);
            }
        ...
        public function getAllCalendarEntries($startdate, $enddate)
        {
            if ($startdate == '' || $enddate == '')
            {
                throw new Exception('Calendar: getAllCalendarEntries requires two dates to be passed in Mysql format');
            }

            $allentries = array();

            $sql = "SELECT ID FROM Calendar WHERE DateDisplay BETWEEN :startdate AND :enddate ORDER BY DateDisplay ASC";
            ...
            {
                $allentries[] = $this->getItemById($story['ID']);
            }
        ...
        /**
         * Delete the complete calendar item - including any images
         *
         * @return mixed
         */
        public function deleteItem()
        {
            ...
            {
                throw new Exception('Class Calendar requires the calendar item ID to be set if you are trying to delete the item');
            }

            //Now delete the item from the DB
            try
            {
                $stmt = $this->_dbconn->prepare("DELETE FROM Calendar WHERE ID = :id LIMIT 1");
                $result = $stmt->execute([
                                             'id' => $this->_id
                                         ]);
            } catch (Exception $e)
            {
                error_log("Failed to delete Calendar record: " . $e);
            }
        ...
        /**
         * Function to check if a similar URL already exists in the Letter table
        ...
        public function URLTextValid($ID = 0, $ContentURL)
        {
            ...
            {
                throw new Exception('Calendar needs the new URL specifying as a string');
            }


            if (clean_int($ID) > 0)
            {
                $sql = "SELECT ID FROM Calendar WHERE URLText = :urltext AND ID != :id";
                $vars = array('urltext' => $ContentURL, 'id' => $ID);
            }
            else
            {
                $sql = "SELECT ID FROM Calendar WHERE URLText = :urltext AND (ID IS NULL OR ID <= 0)";
        ...
        function searchContent($needle = '')
        {
            ...
            //Return IDs
            $sql = "SELECT ID FROM Calendar WHERE (Title LIKE :needle OR Content LIKE :needle) AND DateDisplay <= NOW()";
            $stmt = $this->_dbconn->prepare($sql);
            $stmt->execute([
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC))
            {
                ...
                if ($content['URLText'] != '')
                {
                    $link = "//" . SITEFQDN . "/calendar/" . $content['URLText'];
                }
                else
                {
                    $link = "//" . SITEFQDN . "/content/calendar-detail.php?id=" . $content['ID'];
                }
        ...
        {
            if ($panelexpire == '' || $panelexpire == '0000-00-00' || $panelexpire == '0000-00-00 00:00:00') { $panelexpire = null; }
        ...
        {
            if ($datedisplay == '' || $datedisplay == '0000-00-00' || $datedisplay == '0000-00-00 00:00:00') { $datedisplay = null; }
