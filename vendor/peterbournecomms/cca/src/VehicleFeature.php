<?php
     ...
     *  Relies on VehicleFeatures in database
     *
     *  History
    ...
    class VehicleFeature
    {
        ...
        public function __construct($id = null)
        {
            ...
            if (isset($id) && !is_numeric($id)) {
                throw new Exception('CCA\VehicleFeature->__construct() requires id to be specified as an integer - if it is specified at all');
            }
        ...
        public function getItemById($id = 0){
            ...
            if (!is_numeric($id) || $id <= 0) {
                error_log('CCA\VehicleFeature->getItemById() Unable to retrieve details as no ID set');
                return false;
            }

            //Now retrieve the extra fields
            try {
                $stmt = $this->_dbconn->prepare("SELECT ID, VehicleID, AES_DECRYPT(Title, :key) AS Title, DisplayOrder FROM VehicleFeatures WHERE ID =  :id LIMIT 1");
                $stmt->execute([
            ...
            {
                error_log("CCA\VehicleFeature->getItemById() Failed to retrieve details" . $e);
            }
        ...
        {
            $basesql = "SELECT ID FROM VehicleFeatures WHERE ";
            $order = " ORDER BY VehicleFeatures.DisplayOrder ASC";
            $params = array();

            //Build SQL depending on passedMode and passedNeedle
            switch($passedMode) {
                case 'title':
                    $query = "CONVERT(AES_DECRYPT(VehicleFeatures.Title, :key) USING utf8) LIKE :needle ";
                    $order = "ORDER BY AES_DECRYPT(VehicleFeatures.Title, :key) ASC";
                    $params['needle'] = $passedNeedle."%";
                    $params['key'] = AES_ENCRYPTION_KEY;
                    break;
                    
                case 'vehicle-id':
                    $query = "VehicleFeatures.VehicleID = :needle ";
                    $params['needle'] = $passedNeedle;
                    break;
            ...
            if (isset($passedVehicleID) && is_numeric($passedVehicleID) && $passedVehicleID > 0) {
                $query .= " AND (VehicleFeatures.VehicleID = :vehicle_id) ";
        ...
        public function deleteItem($id = 0)
        {
            ...
            if (!is_numeric($id) || $id <= 0) {
                error_log('CCA\VehicleFeature->deleteItem() Unable to delete as no id set');
            ...
            try {
                $stmt = $this->_dbconn->prepare("DELETE FROM VehicleFeatures WHERE ID = :id LIMIT 1");
                $result = $stmt->execute([
                                             'id' => $id
                                         ]);
                if ($result === true) {
                ...
                error_log("CCA\VehicleFeature->deleteItem() Failed to delete record" . $e);
            }
        ...
        {
            try {
                $stmt = $this->_dbconn->prepare("INSERT INTO VehicleFeatures SET Title = null");
                $stmt->execute();
                ...
                error_log("CCA\VehicleFeature->createNewItem() Failed to create new stub".$e);
            }
        }
        ...
        public function saveItem()
        {
            ...
            try {
                $stmt = $this->_dbconn->prepare("UPDATE VehicleFeatures SET VehicleID = :vehicle_id, Title = AES_ENCRYPT(:title, :key), DisplayOrder = :display_order WHERE ID = :id LIMIT 1");
                $result = $stmt->execute([
                    'key' => AES_ENCRYPTION_KEY,
                ...
                error_log("CCA\VehicleFeature->saveItem() Failed to save record: " . $e);
            }
        ...
        public function updateField($field, $value, $recid) {
            if (!is_numeric($recid)) {
                error_log('CCA\VehicleFeature->updateField() - requires record id to be passed');
                return false;
            }
            if (!is_string($field) || $field == '' ) {
                error_log('CCA\VehicleFeature->updateField() - requires field to be passed');
                return false;
            }
            
            //Check its not one of our special columns
            if ($field == 'ID') {
                error_log('CCA\VehicleFeature->updateField() - Attempted to update prohibited field');
                return false;
            }
            
            
            //Check if the field and record exists
            $stmt = $this->_dbconn->prepare("SELECT ID FROM VehicleFeatures WHERE ID = :id LIMIT 1");
            $stmt->execute([
                'id' => $recid
            ]);
            $item = $stmt->fetch();
            ...
            if ($item['ID'] <= 0 ) {
                error_log('CCA\VehicleFeature->updateField() - Record does not exist');
                return false;
            }
            
            $stmt = $this->_dbconn->prepare("SHOW COLUMNS FROM VehicleFeatures LIKE :field");
            $stmt->execute([
                'field' => $field
            ]);
            $items = $stmt->fetchAll();
            ...
            if (!is_array($items) || count($items) <= 0) {
                error_log('CCA\VehicleFeature->updateField() - Field does not exist');
                return false;
            }
            ...
            //Check if the field and record exists
            if ($field == 'VehicleID' || $field == 'DisplayOrder') {
                //No encryption
                $sql = "UPDATE VehicleFeatures SET ".$field." = :value WHERE ID = :id LIMIT 1";
                $params = array('id' => $recid, 'value' => $value);
            } else {
                $sql = "UPDATE VehicleFeatures SET ".$field." = AES_ENCRYPT(:value, :key) WHERE ID = :id LIMIT 1";
